...
*/
var user = User(name: "Patrick", age: 28, height: 5.9, weight: 150)
...
struct Distance{
    ...
    init(meters:Double, feet:Double){
        self.meters = nil ?? feet * 3.28084
        self.feet = nil ?? meters / 3.28084
    }
}
...
var mile = Distance(meters: 1600, feet: 0)
print(mile) // ????????
...
struct RunningWorkout {
   ...
   var elevation: Double
    ...
    func averageMileTime(distance: Double, time: Double) -> Double {
        let average = time / (distance / 1600)
        return average
    }
}

let result = RunningWorkout(distance: 3200, time: 60, elevation: 600)

print(result.averageMileTime(distance: 3200, time: 60))
//Like that?
...
var run = Steps(steps: 9999, goal: 100000)
print(run)
run.takeStep()
print(run)
